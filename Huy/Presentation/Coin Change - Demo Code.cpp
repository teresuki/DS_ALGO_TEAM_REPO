...
// construct the table and the optimal solution
// by dynamic programming
int coin(int N, vector<int>& optimal_solution)
{
    // assume that n = 4
    int d[4] = {1, 10, 25, 100};
    int t[4][N + 1];
...
{
    int N;
    vector<int> optimal_solution(4, 0);     // to contain the optimal solution generated by DP

    cout << "Enter the amount of money needed to pay back: N = ";
    cin >> N;

    int result = coin(N, optimal_solution);   // call only 1 because of the global container optimal_solution will change each call
